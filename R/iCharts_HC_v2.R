...
InteractiveChart.hc.v2 <- function(dat, xvalue, yvalue, 
  ...
  if (plottype %in% c("scatter","bubble")) {
   ...
   # Adding Regression Line #
    ...
    plottitle <- paste("Histogram displaying the distribution of", xvalue, "as X-axis, group by", groupcolor)       
  
  h1 <- rCharts::hPlot(data = rdat[,xvalue], 
                       title = plottitle)    
...
inlineChart.hc.v2 <- function(dat, xvalue, yvalue, plottype, pointcolor, groupcolor, pointsize, groupsize, addRegline){ 
  p1 <- InteractiveChart.hc.v2(dat, xvalue, yvalue, plottype, pointcolor, groupcolor, pointsize, groupsize, addRegline) 
  p1$set(height = 650) 
